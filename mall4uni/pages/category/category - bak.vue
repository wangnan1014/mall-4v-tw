...
<!-- 滚动内容区 -->
  <view class="main">
    ...
    <scroll-view scroll-y="true" class="leftmenu">
      ...
      <view v-if="!categoryList || !categoryList.length" class="ca-empty">暫無分類</view>
    </scroll-view>
    <!-- 左侧菜单end -->
    ...
    <!-- <block wx:for='{{ productList}}' wx:key=''> -->
      ...
      <view v-if="!productList.length" class="cont-item empty">{{categoryList && categoryList.length ? '該分類下暫無商品' : '暫無商品'}}</view>
     <!-- </block> -->
    </scroll-view>
    <!-- 右侧内容end -->
    <!-- </block> -->
