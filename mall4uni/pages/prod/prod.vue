...
<view class="container">
  ...
  <!-- <view class="cmt-wrap">
    <view class="cmt-tit" @tap="showComment">
      <view class="cmt-t">
        評價
        <text class="cmt-good">好評{{prodCommData.positiveRating}}%</text>
      </view>
      <view class="cmt-count">
        共{{prodCommData.number}}條
    ...
    <view class="cmt-cont">
      ...
      <view class="cmt-more-v" v-if="prodCommPage.records.length > 2">
        <text @tap="showComment">查看全部評價</text>
      </view>
    </view>
  </view> -->
  <!-- 商品详情 -->
    ...
    <view class="pup-sku-main">
      <view class="pup-sku-header">
        ...
        <view class="pup-sku-prop">
          <text>已選</text> {{selectedProp.length>0?selectedProp+'，':''}}{{prodNum}}件
        </view>
      ...
      <view class="pup-sku-body">
        ...
        <view class="pup-sku-area">
          <block>
            <view class="sku-kind">訂餐日期</view>
            <view class="sku-choose">
              <block v-for="(item, index) in dayArr" :key="index">
                <text :class="'sku-choose-item ' + (wxs.array_contain(selectedDay,item)?'active':'') + ' '" :data-ok="true" @tap="toChooseItem1" data-key="orderUseDate" :data-val="item">{{item}}</text>
              </block>
            </view>
          </block>
        </view>		
...
<!--        <view class="btn cart" @tap="addToCart">加入购物车</view>
 -->        <view class="btn buy" @tap="buyNow">立即訂餐</view>
      </view>
    </view>
  </view>
  <!-- 评价弹窗 -->
  <view class="cmt-popup" v-if="commentShow">
    <view class="cmt-tit">
      <view class="cmt-t">
        商品評價
        <text class="cmt-good">好評度{{prodCommData.positiveRating}}%</text>
      </view>
    ...
    <view class="cmt-cont">
      <view class="cmt-tag">
        ...
        <text @tap="getProdCommPage" data-evaluate="0" :class="evaluate==0?'selected':''">好評({{prodCommData.praiseNumber}})</text>
        <text @tap="getProdCommPage" data-evaluate="1" :class="evaluate==1?'selected':''">中評({{prodCommData.secondaryNumber}})</text>
        <text @tap="getProdCommPage" data-evaluate="2" :class="evaluate==2?'selected':''">差評({{prodCommData.negativeNumber}})</text>
        <text @tap="getProdCommPage" data-evaluate="3" :class="evaluate==3?'selected':''">有圖({{prodCommData.picNumber}})</text>
      </view>
      <view class="cmt-items">
        <block v-if="prodCommPage.records.length">
          <view v-for="(item, index) in prodCommPage.records" :key="index" class="cmt-item">
            ...
            <view class="cmt-reply" v-if="item.replyContent">
              <text class="reply-tit">店鋪回復：</text> {{item.replyContent}}
            </view>
          </view>
        </block>
        <view v-if="!prodCommPage.records.length" class="empty">暫無評價</view>
      </view>
      <view class="load-more" v-if="prodCommPage.pages > prodCommPage.current">
        <text @tap="getMoreCommPage">點擊加載更多</text>
      </view>
    </view>
  </view>
</view>
...
export default {
  ...
  onLoad: function (options) {
	...
	// 查看用户是否关注
    //this.getCollection();
	console.log(this.selectedPropObj)
  },
  ...
  methods: {
	...
	loaddays(dayNum){
		...
		for(let i=0;i<dayNum;i++){ 
			//把未来几天的时间放到数组里 
			let odate = new Date(curDate.getFullYear(),curDate.getMonth(),curDate.getDate() + i)
			arr.push(odate.getFullYear()+"-"+(odate.getMonth()+1)+"-"+odate.getDate());	
	    } 
	   this.setData({
		 dayArr: arr
	   });
    ...
    groupSkuProp: function () {
      ...
      for (var i = 0; i < skuList.length; i++) {
        ...
        this.setData({
          ...
          propKeys: propKeys,
		  selectedDay:[this.dayArr?this.dayArr[0]:""]
        });
		console.log(this.selectedPropObj)
	...
	toChooseItem1: function (e) {
	  ...
	  var val = e.currentTarget.dataset.val;
	  var key = e.currentTarget.dataset.key;
	  var sd = this.selectedDay;
	  //多選項
	  var find = false;
	  for(var i = 0; i < sd.length; i++){
		if (sd[i] == val) {//如果要求数据类型也一致，这里可使用恒等号===
			sd.splice(i, 1);
			find = true;
			break;
		}
