...
<!--    <text @tap="onStsTap" data-sts="1" :class="sts==1?'on':''">待支付</text>
	...
	<text @tap="onStsTap" data-sts="7" :class="sts==7?'on':''">已訂餐</text>
	<text @tap="onStsTap" data-sts="8" :class="sts==8?'on':''">已使用</text>
	<text @tap="onStsTap" data-sts="6" :class="sts==6?'on':''">已取消</text>
      ...
      <view class="prod-item">
        <view class="order-num">
          ...
          <view class="order-state">
            <text :class="'order-sts  ' + (item.status==1?'red':'') + '  ' + ((item.status==7||item.status==8)?'gray':'')">{{item.status==7?'已訂':(item.status==8?'已使用':(item.status==1?'待支付':(item.status==2?'待發貨':(item.status==3?'待收貨':(item.status==5?'已完成':'已取消')))))}}</text>
        ...
        <view class="total-num">
		  <view class="useDate"><text>{{"預定日期："+formatDate(new Date(item.orderUseDate)) +""}}</text></view>
          <text class="prodcount">共1件</text>
        ...
        <view class="prod-foot">
          <view class="btn">
            <text v-if="item.status==7" class="button" @tap="onCancelOrder" :data-ordernum="item.orderNumber" hover-class="none">取消訂單</text>
