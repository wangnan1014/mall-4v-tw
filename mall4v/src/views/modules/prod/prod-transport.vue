      ...
      <el-table :data="transportInfo.transfeeFrees"
        ...
        <el-table-column prop="freeType"
          ...
          <template slot-scope="scope">
            ...
            <span v-if="scope.row.freeType === 1">满金額包邮</span>
            <span v-if="scope.row.freeType === 2">满件/重量/体积且满金額包邮</span>
          </template>
        </el-table-column>
        <el-table-column prop="amount">
          <template slot-scope="scope">
            <span v-if="scope.row.freeType === 1">满{{scope.row.amount}}元金額包邮</span>
            <span v-if="scope.row.freeType === 0">满{{scope.row.piece}}件/重量/体积包邮</span>
            <span v-if="scope.row.freeType === 2">满{{scope.row.piece}}件/重量/体积且满{{scope.row.amount}}元金額包邮</span>
          </template>
        </el-table-column>
      </el-table>
    </el-form-item>
